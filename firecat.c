 ...
 *
 * This version is a fork and as such does not represent the work of Bishop Fox i.e. Stach & Liu LLC
 *
 */
#define VERSION "0.1 (fork)"
    ...
    while(1) {
        ...
        if (FD_ISSET(fd1, &er)) {
            if (recv(fd1, &c, 1, MSG_OOB) < 1)
                return 1;
            else if (send(fd2, &c, 1, MSG_OOB) < 1) {
                    perror("ERROR: send()");
                    return 1;
            }
        }
        ...
        if (FD_ISSET(fd2, &er)) {
            if (recv(fd2, &c, 1, MSG_OOB) < 1)
                return 1;
            else if (send(fd1, &c, 1, MSG_OOB) < 1) {
                    perror("ERROR: send()");
                    return 1;
            }
        }
...
static HANDLE StartShell(HANDLE ShellStdinPipeHandle, HANDLE ShellStdoutPipeHandle) {
    ...
    if (CreateProcess(NULL, cmdshell, NULL, NULL, TRUE, 0, NULL, NULL,
...
static VOID SessionReadShellThreadFn(LPVOID Parameter) {
    ...
    /* this bogus peek is here because win32 won't let me close the pipe if it is in waiting for input on a read. */
 ...
 * our own error strings to print.
*/
char *winsockstr(int error) {
    switch (error) {
